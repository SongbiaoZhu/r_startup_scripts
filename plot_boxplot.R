#' Plot Boxplot in Nature Journal Style
#'
#' This function plots a boxplot from a dataset, producing a publication-ready figure
...
#' @param output_file Name of the file where the plot will be saved (e.g., "boxplot.png").
...
#'
#' @return A publication-ready boxplot saved as a PNG file.
plot_boxplot <- function(data,
  ...
  p <- ggplot(data, aes_string(x = x_var, y = y_var)) +
    geom_boxplot(aes(fill = fill_var), outlier.shape = NA) +  # Adjust fill aesthetic
    stat_summary(
      fun = "median",
      geom = "point",
      shape = 21,
...
if (interactive()) {
  ...
  plot_boxplot(
    ...
    output_file = "boxplot_by_sample.png",
    x_label = "Sample",
    y_label = "Log2 Abundance",
    title = "Boxplot of Abundance by Sample"
  )
  ...
  plot_boxplot(
    ...
    output_file = "boxplot_by_group.png",
    x_label = "Sample",
    y_label = "Log2 Abundance",
    title = "Boxplot of Abundance by Group"
  )
}
