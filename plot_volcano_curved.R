#' Plot Volcano with Curved Threshold in Nature Journal Style
#'
#' This function plots a volcano plot with a curved threshold, producing a publication-ready figure
...
#' @param output_file Name of the file where the plot will be saved (e.g., "volcano_plot_curved.png").
...
#' @param a Constant to control the curvature of the threshold (default is 0.05).
#'
#' @return A publication-ready volcano plot saved as a PNG file.
plot_volcano_curved <- function(data,
                                log2fc,
                                p_value,
                                output_file,
                                x_label = "Fold Change",
  ...
  library(ggplot2)
  library(dplyr)
  library(rlang)  # Added for using sym()
  
  # Nature journal style settings
  text_size <- 14  # Set default text size
  figure_size <- c(6, 4.5)  # Set default figure size in inches
  
  # Add significance and regulation status based on the curved threshold
  log2fc_sym <- sym(log2fc)
  p_value_sym <- sym(p_value)
  
  data <- data %>%
    dplyr::mutate(Regulation = dplyr::case_when(
      (!!log2fc_sym)^2 > a * (-log10(!!sym(p_value))) & !!log2fc_sym > 0 ~ "Upregulated",
      (!!log2fc_sym)^2 > a * (-log10(!!sym(p_value))) & !!log2fc_sym < 0 ~ "Downregulated",
      TRUE ~ "Not Significant"
    ))
  
  # Generate the volcano plot
  p <- ggplot(data, aes_string(x = log2fc, y = sprintf("-log10(%s)", p_value), color = "Regulation")) +
    geom_point(alpha = 0.6, size = 1.5) +
    scale_color_manual(values = c("Upregulated" = "#D55E00", "Downregulated" = "#0072B2", "Not Significant" = "#999999")) +
    labs(
      title = title, 
      x = bquote(log[2]*"("*.(x_label)*")"),  # log2(Fold Change)
